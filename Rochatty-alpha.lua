...
-- Cache for AI-generated messages
local aiSentMessages = {}
...
local function filterAndBypassChunk(chunk)
    ...
    if isFiltered(chunk) then
        ...
        NotificationLibrary:SendNotification("Success", "Filtered chunk detected and bypassed", 3)
...
-- Function to send AI-generated chat messages
local function sendAIChatMessage(message)
    table.insert(aiSentMessages, message) -- Track the message as AI-sent
...
local function sendResponseInChunks(chunks)
    for _, chunk in ipairs(chunks) do
        ...
        for _, processedChunk in ipairs(processedChunks) do
            sendAIChatMessage(processedChunk)
...
-- Function to listen for AI-sent filtered messages and reprocess them
local function listenForFilteredMessagesAndQueue()
    TextChatService.TextChannels.RBXGeneral.MessageReceived:Connect(function(data)
        local message = data.Text
        local sender = Players:GetPlayerByUserId(data.TextSource.UserId)

        -- Ensure the message is sent by the LocalPlayer (you)
        if sender ~= Players.LocalPlayer then
            return
        end

        -- Check if the message was generated by the AI
        local isAISent = table.find(aiSentMessages, message) ~= nil
        if not isAISent then
            -- This message was not generated by the AI, so skip it
            if Config.Debug then
                print("Ignored non-AI message: " .. message)
            end
            return
        end
        ...
        if isFiltered(message) or message:match("^#+$") then
            warn("Filtered AI message detected. Splitting, bypassing, and re-sending...")

            NotificationLibrary:SendNotification(
                "Filtering Detected",
                "Processing filtered message with bypass and splitting...",
                3
            )

            -- Initial splitting into manageable chunks
            local originalText = message -- Use the original message as a starting point
            local initialChunks = {}
            local chunkSize = math.floor(Config.ChatCharacterLimit / 2) -- Smaller chunks for safety
            for i = 1, #originalText, chunkSize do
                table.insert(initialChunks, originalText:sub(i, i + chunkSize - 1))
            end
            ...
            for _, chunk in ipairs(initialChunks) do
                ...
                if not processFilteredChunk(chunk, originalText) then
                    warn("Skipping chunk as all attempts failed: " .. chunk)
                end
            end

            -- Mark the message as processed
            table.remove(aiSentMessages, table.find(aiSentMessages, message)) -- Remove the message from the AI cache
        else
            -- Message is not filtered, no action needed
            if Config.Debug then
                print("Unfiltered AI message detected: " .. message)
            end
            table.remove(aiSentMessages, table.find(aiSentMessages, message)) -- Clean up AI cache
        end
    end)
end
